...
class SelfPlay:
            ...
            if not test_mode:
                if hasattr(self.config, "num_simulations_fn"):
                    training_step = ray.get(
                        shared_storage.get_info.remote("training_step"))
                    num_simulations = self.config.num_simulations_fn(
                        training_step)
                else:
    ...
    def play_game(
        ...
        try:
            random_move_till_n_action = getattr(
                self.config, "random_move_till_n_action_in_self_play")
        except AttributeError:
            random_move_till_n_action = 0
        ...
        with torch.no_grad():
            while (
                ...
                current_legal_actions = self.game.legal_actions()
                if len(current_legal_actions) == 0:     
                    done = True 
                    game_history.legal_actions_history.append([])
                    break
                assert (
